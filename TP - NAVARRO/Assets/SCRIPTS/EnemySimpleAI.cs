...
public class EnemySoldier : MonoBehaviour
{
    ...
    [SerializeField] private float attackRate = 1.0f;   // golpes por segundo
    ...
    void Awake()
    {
        ...
        if (!player)
        {
            ...
            else { var p = GameObject.FindWithTag("Player"); if (p) player = p.transform; if (detector && player) detector.target = player; }
        }
    ...
    private void TryAttackPlayer()
    {
        ...
        // no pegar si hay pared
        bool clear = true;
        if (detector)
        {
            LayerMask obsMask = detector.obstacleMask;
            if (obsMask != 0 &&
                Physics.Raycast(new Ray(eye, dir.normalized), out RaycastHit hit, attackRange, obsMask, QueryTriggerInteraction.Ignore))
                clear = false;
        }
        if (!clear) return;
